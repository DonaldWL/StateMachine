...
class _CCSharpLan(_CBase):
  ...
  def __init__(self, Language, TPLDir, STMDir, OverWrite, LogFh, SMSResult, Optimize):  
    ...
    if Language != 'CSharp':
      raise AttributeError('_CLan only supports CSharp (C#) Code, language passed in ({0})'.format(Language))
  ...
  def _StartState(self):
    ...
    StartState tag
    '''
    self._STMFileFh.write((' ' * self._ForcedOffset) + 'public const int StartStateIndx = ' + str(self._StartStateIndx) + ';\n')
  ...
  def _EndState(self):
    ...
    EndState tag
    '''
    self._STMFileFh.write((' ' * self._ForcedOffset) + 'public const int EndStateIndx = ' + str(self._EndStateIndx) + ';\n')
  ...
  def _StateTable(self):
    ...
    '''
      ...
      # Create the table.
    Line = (' ' * self._ForcedOffset) + 'public const int STLen = ' + str(self._STLen) + ';\n'
    self._STMFileFh.write(Line)
    Line = ' ' * self._ForcedOffset + 'public static readonly int[] StateTable = {'
    tLine = ''
    LineIndent = ' ' * len(Line)
    StateName = ''
  ...
  def _CodeBlockNames(self): 
    ...
    Line = (' ' * self._ForcedOffset) + 'public const int CBLen = ' + str(len(self._SMSResult.CodeBlockNames)) + ';\n'
    self._STMFileFh.write(Line)
    
      # Create the CodeBlockNames variables
    Line = ' ' * self._ForcedOffset + 'public static readonly string[] CodeBlockNames = {'
    LineIndent = ' ' * len(Line)
  ...
  def _CodeBlockValues(self):
    ...
    Line = ''
    for Indx in range(0, len(self._SMSResult.CodeBlockNames)):
      Line += '{0}public const int CB_{1} = {2};\n'.format(LineIndent, 
  ...
  def _STIValues(self):
    LineIndent = (' ' * self._ForcedOffset)
    Line = LineIndent + 'public const int STI_CBIdx = 0;\n' 
    if self._Optimize:
      Line += LineIndent + 'public const int STI_StatesIdx = 1;\n' 
    else:
      Line += LineIndent + 'public const int STI_StateIdx = 1;\n' 
      Line += LineIndent + 'public const int STI_StateLenIdx = 2;\n' 
      Line += LineIndent + 'public const int STI_StatesIdx = 3;\n' 
    self._STMFileFh.write(Line)
  ...
  def _StateNames(self):
    ...
    Line = (' ' * self._ForcedOffset) + 'public const int SNLen = ' + str(len(self._SMSResult.StateNames)) + ';\n'
    self._STMFileFh.write(Line)
    
      # Create the StateNames variables
    Line = ' ' * self._ForcedOffset + 'public static readonly string[] StateNames = {'
    LineIndent = ' ' * len(Line)
  ...
  def _StateValues(self):
    ...
    Line = ''
    for Indx in range(0, len(self._SMSResult.StateNames)):
      Line += '{0}public const int ST_{1} = {2};\n'.format(LineIndent, 
