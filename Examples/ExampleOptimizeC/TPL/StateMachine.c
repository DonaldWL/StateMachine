...
/* @@RemoveLine@@
  @@ReplaceLine:2@@ int CurStateIndx = @@StartStateValue@@;
@@RemoveLine@@ */
  int CurStateIndx = 0;  /*@@RemoveLine@@*/
  ...
  while (ProcessStates) {
    switch (StateTable[CurStateIndx + STI_CBIdx]) {
      ...
      default:
        ReturnValue->UserRValue = -1;
        ReturnValue->UserData = BuildMsg("Invalid CodeBlock => State: {SN}  CodeBlock: {BN}  StateRValue: {RV}",
                                          PrevCurStateIndx, StateRValue);
        Log("Error", 1, (char *) ReturnValue->UserData);
        ProcessStates = false;
        break;
    }
    ...
    if (TraceFh != NULL) {
      ...
      char CurState[20];
      _itoa_s(CurStateIndx, CurState, 20, 10);
      char CB[20];
      _itoa_s(StateTable[CurStateIndx + STI_CBIdx], CB, 20, 10);
      char RValue[20];
      _itoa_s(StateRValue, RValue, 20, 10);

        // Format is yyyy-mm-dd HH:MM:SS: <StateIndx>,<CBIndx>,<StateRValue>
      Msg = StringBuild(NULL, 7, tDateTime, CurState, ",", CB, ",", RValue, "\n");
      fprintf(TraceFh, Msg);
      free(Msg);
    }
  ...
  for (size_t i = 0; i < MsgTLen; i++) {
    if (MsgT[i] == '{') {
      ...
      if (MsgT[i] == '}' && ii < 100) {
        ...
        if (!strcmp(Cmd, "SN")) {
          char TheString[100];
          _itoa_s(CurStateIndx, TheString, 100, 10);
          Msg = StringBuild(Msg, 1, TheString);
        } else if (!strcmp(Cmd, "BN")) {
          char TheString[100];
          _itoa_s(StateTable[CurStateIndx + STI_CBIdx], TheString, 100, 10);
          Msg = StringBuild(Msg, 1, TheString);
