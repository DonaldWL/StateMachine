...
extern bool FileExist(const char *_path);
  ...
  // If the path exists and is a dir will return true
extern bool IsDir(const char *_path);

  // If the path exists and is a file will return true
extern bool IsFile(const char *_path);

  // Returns the current directory.  You must manage
  // the memory.  free it when done.
...
extern void FileDListDeleter(void *_userData);
  ...
  // Used to remove a buffer.  If the buffer is removed the string returned is
  // NULL else its a string that you must manage.  Free when done.
extern char *RemoveDir(const char *_fileName);

  // Used to see if FILE isa atty.
