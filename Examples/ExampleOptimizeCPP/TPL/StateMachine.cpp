...
int CStateMachine::Run(void)
{
  ...
  while (ProcessStates) {
    switch (StateTable[CurStateIndx + STI_CBIdx]) {
      ...
      default:
        throw SMCodeBlockInvalid(CurStateIndx, StateTable[CurStateIndx + STI_CBIdx], StateRValue);
    };
    ...
    if (TraceFh != nullptr) {
      ...
      strftime(buffer, 30, "%F %T: ", &newtime);
        ...
        // Format is yyyy-mm-dd HH:MM:SS: <StateIndx>,<codeblocknum>,<StateRValue>
      *TraceFh << buffer << CurStateIndx  << "," << StateTable[CurStateIndx + STI_CBIdx] << "," <<
